...
def _load_env_file() -> None:
    """Local adaptation: load credentials from a .env file next to this skill.

    Search order: skill root .env, then ~/.config/paddleocr/.env.
    Only fills variables that are not already set in the environment.
    """
    candidates = [
        Path(__file__).resolve().parent.parent / ".env",  # skill root
        Path.home() / ".config" / "paddleocr" / ".env",
    ]
